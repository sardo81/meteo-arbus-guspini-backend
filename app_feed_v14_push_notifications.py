...
api = FastAPI(title="Meteo Arbus-Guspini API", version="14.0-push-notifications")
api.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
VAPID_SUBJECT = os.getenv("VAPID_SUBJECT", "https://meteo-arbus-guspini.netlify.app").strip()
PUBLIC_APP_URL = os.getenv("PUBLIC_APP_URL", "https://meteo-arbus-guspini.netlify.app").strip().rstrip("/")
ROME_TZ = ZoneInfo("Europe/Rome")
NOTIFICATION_LOOKAHEAD_HOURS = 24
NOTIFICATION_SENT_RETENTION_HOURS = 72
...
def _push_configured() -> bool:
    return bool(_cloud_configured() and VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY and VAPID_SUBJECT)
...
def _normalize_string_list(value: Any, allowed: set[str], default: list[str]) -> list[str]:
...
def _store_push_subscription(payload: Dict[str, Any]) -> Dict[str, Any]:
    subscription = payload.get("subscription") if isinstance(payload, dict) else None
    if not isinstance(subscription, dict):
        raise HTTPException(status_code=400, detail="subscription mancante")
    endpoint = str(subscription.get("endpoint") or "").strip()
    keys = subscription.get("keys") if isinstance(subscription.get("keys"), dict) else {}
    p256dh = str(keys.get("p256dh") or "").strip()
    auth = str(keys.get("auth") or "").strip()
    if not endpoint or not p256dh or not auth:
        raise HTTPException(status_code=400, detail="Sottoscrizione push incompleta")

    places = _normalize_string_list(payload.get("places"), {"Arbus", "Guspini"}, ["Arbus"])
    categories = _normalize_string_list(
        payload.get("categories"),
        {"thunder", "heavy_rain", "wind", "heat"},
        ["thunder", "heavy_rain", "wind", "heat"],
    )
    ...
    result = _supabase_request(
        f"{PUSH_SUBSCRIPTIONS_TABLE}?on_conflict=id",
        method="POST",
        payload=[row],
        prefer="resolution=merge-duplicates,return=representation",
    )
    return {"status": "ok", "subscription_id": row["id"], "places": places, "categories": categories, "rows": len(result or [])}


def _active_push_subscriptions() -> List[Dict[str, Any]]:
    rows = _supabase_request(
        f"{PUSH_SUBSCRIPTIONS_TABLE}?active=eq.true&select=id,endpoint,p256dh,auth,places,categories",
        method="GET",
    )
    return rows if isinstance(rows, list) else []


def _deactivate_push_subscription(subscription_id: str) -> None:
    if not subscription_id:
        return
    _supabase_request(
        f"{PUSH_SUBSCRIPTIONS_TABLE}?id=eq.{subscription_id}",
        method="PATCH",
...
def _send_web_push(subscription: Dict[str, Any], message: Dict[str, Any]) -> Dict[str, Any]:
    if not _push_configured():
        raise RuntimeError("Notifiche push non configurate")
    try:
        from pywebpush import webpush, WebPushException
    except Exception as exc:
        raise RuntimeError("Libreria pywebpush non installata sul backend") from exc
    ...
    try:
        ...
        return {"ok": False, "status_code": code, "error": str(exc)[:500]}
    except Exception as exc:
        return {"ok": False, "status_code": None, "error": f"{type(exc).__name__}: {exc}"[:500]}


def _parse_local_forecast_time(value: Any) -> Optional[datetime]:
...
def _human_local_time(dt: datetime, now_local: datetime) -> str:
    delta = (dt.date() - now_local.date()).days
    hhmm = dt.strftime("%H:%M")
    if delta == 0:
        return f"oggi intorno alle {hhmm}"
    if delta == 1:
        return f"domani intorno alle {hhmm}"
    return f"{dt.strftime('%d/%m')} intorno alle {hhmm}"


def _latest_future_archive_rows(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    archive = state.get("archive")
    if not isinstance(archive, list):
        return []
    now_local = datetime.now(ROME_TZ)
    horizon = now_local + timedelta(hours=NOTIFICATION_LOOKAHEAD_HOURS)
    best: Dict[tuple[str, str], Dict[str, Any]] = {}
    for raw in archive:
        if not isinstance(raw, dict):
            continue
        place = str(raw.get("place") or "").strip()
        if place not in {"Arbus", "Guspini"}:
            continue
        valid = _parse_local_forecast_time(raw.get("validTime"))
        if valid is None or valid <= now_local or valid > horizon:
            continue
        key = (place, valid.isoformat())
        issue = _parse_local_forecast_time(raw.get("issueTime"))
        old = best.get(key)
        old_issue = _parse_local_forecast_time(old.get("issueTime")) if old else None
        if old is None or (issue is not None and (old_issue is None or issue > old_issue)):
            copy = dict(raw)
            copy["_valid_dt"] = valid
            best[key] = copy
    return sorted(best.values(), key=lambda row: row.get("_valid_dt") or now_local)
...
def _notification_candidates(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    now_local = datetime.now(ROME_TZ)
    alerts: List[Dict[str, Any]] = []
    for row in _latest_future_archive_rows(state):
        place = str(row.get("place"))
        valid: datetime = row["_valid_dt"]
        when = _human_local_time(valid, now_local)
        local_temp = _n(row.get("localForecast"), -999.0)
        gust = _n(row.get("forecastGust"))
        rain_acc = _n(row.get("forecastRainInterval"))
        p_rain = max(_n(row.get("pRainInterval")), _n(row.get("pRain")))
        p10 = _n(row.get("pRain10"))
        rain_type = str(row.get("forecastRainTypeId") or "").strip().lower()
        cape = _n(row.get("forecastCapeMax"))
        li = _n(row.get("forecastLiftedIndexMin"), 99.0)
        reliability = str(row.get("reliability") or "").strip()

        if rain_type == "convective":
            risk = p_rain + (10 if cape >= 800 else 0) + (10 if li <= -2 else 0) + (8 if rain_acc >= 2 else 0)
            risk = max(0.0, min(100.0, risk))
            if risk >= 40:
                alerts.append({
                    "category": "thunder", "place": place, "valid": valid, "score": risk,
                    "title": f"⛈ Possibile temporale · {place}",
                    "body": f"Segnale convettivo {when}. Rischio locale {risk:.0f}/100, probabilità di pioggia fino al {p_rain:.0f}%.",
                    "tag": f"meteo-thunder-{place.lower()}-{valid.strftime('%Y%m%d%H')}",
                })

        if p10 >= 35 or rain_acc >= 10:
            alerts.append({
                "category": "heavy_rain", "place": place, "valid": valid, "score": max(p10, min(100.0, rain_acc * 4)),
                "title": f"🌧 Pioggia intensa possibile · {place}",
                "body": f"Finestra più delicata {when}. Accumulo previsto circa {rain_acc:.1f} mm; probabilità di almeno 10 mm {p10:.0f}%.",
                "tag": f"meteo-rain-{place.lower()}-{valid.strftime('%Y%m%d%H')}",
            })

        if gust >= 60:
            alerts.append({
                "category": "wind", "place": place, "valid": valid, "score": min(100.0, gust),
                "title": f"💨 Vento forte possibile · {place}",
                "body": f"Raffiche previste fino a circa {gust:.0f} km/h {when}.",
                "tag": f"meteo-wind-{place.lower()}-{valid.strftime('%Y%m%d%H')}",
            })

        if local_temp >= 35:
            level = "molto intenso" if local_temp >= 38 else "intenso"
            extra = f" Affidabilità {reliability.lower()}." if reliability else ""
            alerts.append({
                "category": "heat", "place": place, "valid": valid, "score": min(100.0, 50 + (local_temp - 35) * 12),
                "title": f"🌡 Caldo {level} · {place}",
                "body": f"Temperatura locale prevista circa {local_temp:.0f} °C {when}.{extra}",
                "tag": f"meteo-heat-{place.lower()}-{valid.strftime('%Y%m%d%H')}",
            })

    unique: Dict[str, Dict[str, Any]] = {}
    for alert in alerts:
        key = str(alert["tag"])
        if key not in unique or _n(alert.get("score")) > _n(unique[key].get("score")):
            unique[key] = alert
    return sorted(unique.values(), key=lambda a: a["valid"])


def _notification_sent_map(state: Dict[str, Any]) -> Dict[str, str]:
    value = state.get("notification_sent_keys")
    return value if isinstance(value, dict) else {}


def _prune_notification_sent_map(sent: Dict[str, str]) -> Dict[str, str]:
    cutoff = datetime.now(timezone.utc) - timedelta(hours=NOTIFICATION_SENT_RETENTION_HOURS)
    out: Dict[str, str] = {}
    for key, stamp in sent.items():
        try:
            ...
            if dt >= cutoff:
                out[str(key)] = dt.astimezone(timezone.utc).isoformat()
        except Exception:
            pass
    return out
...
def notifications_config() -> Dict[str, Any]:
    try:
    ...
    return {
        "configured": _push_configured() and library_available,
        "cloud_configured": _cloud_configured(),
        "library_available": library_available,
        "public_key": VAPID_PUBLIC_KEY if VAPID_PUBLIC_KEY else None,
        "categories": ["thunder", "heavy_rain", "wind", "heat"],
        "places": ["Arbus", "Guspini"],
    }


@api.post("/notifications/subscribe")
def notifications_subscribe(payload: Dict[str, Any]) -> Dict[str, Any]:
    if not _push_configured():
        raise HTTPException(status_code=503, detail="Sistema push non configurato")
    try:
        return _store_push_subscription(payload)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=503, detail=f"Salvataggio push non riuscito: {exc}") from exc


@api.post("/notifications/unsubscribe")
def notifications_unsubscribe(payload: Dict[str, Any]) -> Dict[str, Any]:
    endpoint = str(payload.get("endpoint") or "").strip()
    if not endpoint:
        raise HTTPException(status_code=400, detail="endpoint mancante")
    try:
        _deactivate_push_subscription(_subscription_id(endpoint))
    except Exception as exc:
        raise HTTPException(status_code=503, detail=f"Disattivazione push non riuscita: {exc}") from exc
    return {"status": "ok"}


@api.post("/notifications/test")
def notifications_test(payload: Dict[str, Any]) -> Dict[str, Any]:
    if not _push_configured():
        raise HTTPException(status_code=503, detail="Sistema push non configurato")
    subscription = payload.get("subscription") if isinstance(payload, dict) else None
    if not isinstance(subscription, dict):
        raise HTTPException(status_code=400, detail="subscription mancante")
    keys = subscription.get("keys") if isinstance(subscription.get("keys"), dict) else {}
    target = {"endpoint": subscription.get("endpoint"), "p256dh": keys.get("p256dh"), "auth": keys.get("auth")}
    result = _send_web_push(target, {
        "title": "✅ Meteo Arbus–Guspini",
        "body": "Notifiche attive: il test è riuscito.",
        "tag": "meteo-test",
        "url": PUBLIC_APP_URL,
        "icon": f"{PUBLIC_APP_URL}/icon-192.png",
        "badge": f"{PUBLIC_APP_URL}/icon-192.png",
    })
    if not result.get("ok"):
        raise HTTPException(status_code=503, detail=f"Invio test non riuscito: {result.get('error')}")
    return {"status": "ok", "result": result}


@api.get("/automation/notifications/check")
def automation_notifications_check(x_automation_secret: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    if not AUTOMATION_SECRET:
        raise HTTPException(status_code=503, detail="AUTOMATION_SECRET non configurato")
    if not x_automation_secret or not secrets.compare_digest(x_automation_secret, AUTOMATION_SECRET):
        raise HTTPException(status_code=401, detail="Chiave automazione non valida")
    ...
    candidates = _notification_candidates(state)
    sent_map = _prune_notification_sent_map(_notification_sent_map(state))
    new_alerts = [a for a in candidates if a["tag"] not in sent_map]
    subscriptions = _active_push_subscriptions()

    deliveries = failures = deactivated = 0
    marked: List[str] = []

    for alert in new_alerts:
        matched = 0
        for sub in subscriptions:
            places = sub.get("places") if isinstance(sub.get("places"), list) else []
            categories = sub.get("categories") if isinstance(sub.get("categories"), list) else []
            if alert["place"] not in places or alert["category"] not in categories:
                continue
            matched += 1
            result = _send_web_push(sub, {
                "title": alert["title"], "body": alert["body"], "tag": alert["tag"],
                "url": f"{PUBLIC_APP_URL}/", "icon": f"{PUBLIC_APP_URL}/icon-192.png", "badge": f"{PUBLIC_APP_URL}/icon-192.png",
            })
            if result.get("ok"):
                deliveries += 1
            else:
                ...
                if result.get("status_code") in {404, 410}:
                    try:
                        _deactivate_push_subscription(str(sub.get("id") or ""))
                        deactivated += 1
                    except Exception:
                        pass

        if matched > 0:
            sent_map[alert["tag"]] = datetime.now(timezone.utc).isoformat()
            marked.append(alert["tag"])

    state["notification_sent_keys"] = sent_map
    cloud_state_put(state)
    return {
        "status": "ok", "candidates": len(candidates), "new_alerts": len(new_alerts),
        "subscriptions": len(subscriptions), "deliveries": deliveries, "failures": failures,
        "deactivated": deactivated, "alerts_marked": marked,
...
def notifications_status() -> Dict[str, Any]:
    ...
    if _cloud_configured():
        try:
            count = len(_active_push_subscriptions())
            storage_ok = True
        except Exception as exc:
            error = f"{type(exc).__name__}: {exc}"
    return {"configured": _push_configured(), "storage_ok": storage_ok, "active_subscriptions": count, "error": error}
...
def root() -> Dict[str, str]:
    return {
        ...
        "parser": "Weather Underground background feed v14 con archivio stazioni, doppio accumulo pioggia e notifiche push meteo",
        "endpoint": "/stations",
    }
