...
api=FastAPI(title='Meteo Arbus-Guspini API',version='4.0-table')
...
def click_table_view(page:Page)->None:
    try:
        ...
        h.scroll_into_view_if_needed(timeout=8000);page.wait_for_timeout(1000)
    except Exception:pass
    candidates=[page.get_by_text('Table',exact=True),page.get_by_role('button',name=re.compile(r'^Table$',re.I)),page.get_by_role('tab',name=re.compile(r'^Table$',re.I)),page.locator('text=Table')]
    for c in candidates:
        try:
            if c.count():c.first.scroll_into_view_if_needed(timeout=5000);c.first.click(timeout=8000);page.wait_for_timeout(4000);return
        except Exception:continue
    raise RuntimeError('Pulsante Table non trovato')
...
def parse_history_table(rows):
    ...
    return {'source':'wunderground_history_table','parser_version':'4.0-table','status':'table_read','updated':row_value(selected,idx_time),'age_minutes':None,'stale':None,'temperature_c':temp,'dewpoint_c':dew,'humidity_pct':hum,'wind_dir':wind_dir,'wind_speed_kmh':wind,'wind_gust_kmh':gust,'pressure_hpa':pressure,'rain_today_mm':rain,'selected_row':selected,'headers':headers,'quality_warnings':warnings}

def scrape_station(browser:Browser,code:str,meta:Dict[str,str])->Dict[str,Any]:
    out={'station_code':code,'station_name':meta['name'],'url':meta['url'],'captured_at_utc':datetime.now(timezone.utc).isoformat(),'parser_version':'4.0-table','data':None,'errors':[]}
...
@api.get('/')
@api.head('/')
def root():return {'status':'ok','message':'Backend meteo Arbus-Guspini attivo','parser':'Weather History Table v4','endpoint':'/stations'}
@api.get('/health')
@api.head('/health')
