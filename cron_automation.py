...
def http_json(url: str, method: str = "GET", payload=None, headers=None, timeout=90):
    ...
    try:
        ...
        raise RuntimeError(
            f"HTTP {exc.code} su {url}: {detail[:500]}"
        ) from exc

    except URLError as exc:
        raise RuntimeError(
            f"Connessione fallita verso {url}: {exc}"
        ) from exc
...
def merge_meta(state: dict, run_key: str) -> dict:
    meta = (
        state.get("automationMeta")
        if isinstance(state.get("automationMeta"), dict)
        else {}
    )

    completed = (
        meta.get("completedRuns")
        if isinstance(meta.get("completedRuns"), list)
        else []
    )
    ...
    meta["lastSuccessAt"] = datetime.now(ROME).isoformat(
        timespec="seconds"
    )
...
def main() -> int:
    ...
    if not action and not test_requested:
        print(
            f"NOOP {now.isoformat(timespec='seconds')} "
            f"- nessuna operazione prevista"
        )
        return 0
    ...
    state = (
        cloud.get("state")
        if isinstance(cloud.get("state"), dict)
        else {}
    )
    ...
    if not action and test_requested:
        ...
        archive = (
            state.get("archive")
            if isinstance(state.get("archive"), list)
            else []
        )

        print(
            f"TEST START {now.isoformat(timespec='seconds')} "
            f"cloud_history={len(history)} "
            f"cloud_archive={len(archive)}"
        )
    ...
    # Evita di eseguire due volte lo stesso slot.
    if run_key in (meta.get("completedRuns") or []):
        print(
            f"SKIP {run_key} - già completato"
        )
        return 0

    # Importiamo Playwright solo per i sei avvii utili:
    # i NOOP rimangono leggerissimi.
    from playwright.sync_api import sync_playwright

    print(
        f"START {run_key} action={action}"
    )
    ...
    with sync_playwright() as p:
        ...
        # -----------------------------------------------------
        # Carica lo stato cloud nel localStorage dell'app.
        # -----------------------------------------------------
        page.evaluate(
            """([h,a]) => {
        ...
        # -----------------------------------------------------
        # Ripristina impostazioni salvate nel cloud.
        # -----------------------------------------------------
        if settings:
            page.evaluate(
                """(st) => {
        ...
        # -----------------------------------------------------
        # Recupera lo stato aggiornato dal frontend.
        # -----------------------------------------------------
    ...
    # solo dopo il successo delle operazioni meteo.
    # ---------------------------------------------------------
    new_state["automationMeta"] = meta

    new_state = merge_meta(
        new_state,
        run_key,
    )
    ...
    try:
        notification_result = http_json(
            f"{backend}/automation/notifications/check",
            headers={
                "X-Automation-Secret": secret
            },
            timeout=90,
        ...
        print(
            "NOTIFICATIONS "
            f"candidates="
            f"{notification_result.get('candidates', 0)} "
            f"new="
            f"{notification_result.get('new_alerts', 0)} "
            f"sent="
            f"{notification_result.get('deliveries', 0)} "
            f"failures="
            f"{notification_result.get('failures', 0)}"
        )
    ...
    print(
        f"DONE {run_key} "
        f"history="
        f"{len(new_state.get('history', []))} "
        f"archive="
        f"{len(new_state.get('archive', []))} "
        f"cloud="
