...
api = FastAPI(title="Meteo Arbus-Guspini API", version="12.0-full-station-archive")
api.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def parse_observation(obs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "parser_version": "12.0-full-station-archive",
        "status": "feed_read",
...
def _station_observation_row(
    ...
    return {
        ...
        "parser_version": data.get("parser_version") or station_payload.get("parser_version") or "12.0-full-station-archive",
...
def _update_rain_monitor_state(
    ...
    """Aggiorna lo stato persistente dell'evento pioggia per una stazione.

    La grandine non viene dedotta automaticamente: resta "unknown" finché
    non viene aggiunta da una fonte specifica o manualmente.
    """
    ...
    if epoch is None:
        # alcuni feed espongono solo updated in ISO
    ...
    rate = safe_float(data.get("rain_rate_mm_h"))
    ...
    delta = _positive_rain_delta(previous_total, total)
    wet_now = rate >= RAIN_EVENT_START_RATE_MM_H or delta >= RAIN_EVENT_MIN_DELTA_MM
    ...
    if not active and wet_now:
        current = {
            ...
            "end_utc": None,
            "end_epoch": None,
            "accumulation_mm": round(delta, 2),
            "max_rate_mm_h": round(max(0.0, rate), 2),
            "hail": "unknown",
            "status": "active",
            "classification": None,
        ...
        current["accumulation_mm"] = round(
            float(current.get("accumulation_mm") or 0.0) + delta, 2
        )
        current["max_rate_mm_h"] = round(
            max(float(current.get("max_rate_mm_h") or 0.0), max(0.0, rate)), 2
        )
        ...
        if wet_now:
            ...
            if dry_minutes >= RAIN_EVENT_DRY_END_MINUTES:
                ...
                accum = float(current.get("accumulation_mm") or 0.0)
                max_rate = float(current.get("max_rate_mm_h") or 0.0)
                ...
                current.update({
                    ...
                    "classification": _rain_event_classification(
...
def root() -> Dict[str, str]:
    return {
        ...
        "parser": "Weather Underground background feed v12 con archivio automatico completo stazioni, eventi pioggia e correzione grandine",
        "endpoint": "/stations",
    }
